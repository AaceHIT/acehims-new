<?php
//*** URLX » Tydi™ ~ AO™ • @iamodao • www.osawere.com ~ © 2023 • Apache License ***//
class URLX {
	...
	public static function protocol() {
		return HTTPX::isSecure() ? 'https' : 'http';
	}
	...
	public static function prepare(string $link) {
		if (Vars::isEmpty($link)) {
			return false;
		} elseif ($link === 'SELF') {
			$link = self::is('IS');
		} elseif ($link === 'BASE') {
	...
	public static function fixer(string $baseURL, string $req, string $compareURL = 'SELF', $uri = true, $delay = false) {
		...
		$compareURL = self::prepare($compareURL);
