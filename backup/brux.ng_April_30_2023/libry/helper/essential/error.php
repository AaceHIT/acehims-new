<?php
//*** ErrorX » Tydi™ ~ AO™ • @iamodao • www.osawere.com ~ © 2023 • Apache License ***//
...
class ErrorX {
	...
	public static function data($e = '') {
		...
		// • Error Sets
		$dataset = ErrorX::set($code);

		// • Update Error Sets
		if (ArrayX::isNotKeyOrEmpty($dataset, 'status')) {
			if (Vars::isNotEmpty($status)) {
		...
		if (Vars::isNotEmpty($author)) {
			$dataset['author'] = $author;
		} elseif ($dataset['status'] === 'F9') {
			$dataset['author'] = 'GENRIQ';
		}

		if (Vars::isNotEmpty($id)) {
			$dataset['errorid'] = $id;
		}

		if (Vars::isNotEmpty($code)) {
			$dataset['code'] = $code;
		}

		if (Vars::isNotEmpty($title)) {
			$dataset['title'] = $title;
		}

		if (Vars::isNotEmpty($message)) {
