...
class SettingApp extends Appzr {
	...
	public function profileUpdate() {
		$sessionUser = $this->logic['sessionUser'];
		if (App::isPost()) {
			...
			$resolve = $this->auth->updateSessionUser($sessionUser['puid'], $input);
			if ($resolve === 'UPDATED') {
	...
	public function changePassword() {
		...
		if (App::isPost()) {
			...
			if ($input['password'] === $input['repassword']) {
				...
				$resolve = $this->auth->changePassword($sessionUser['puid'], $password);
				if ($resolve === 'UPDATED') {
