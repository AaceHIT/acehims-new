...
class AssetQ {
	...
	public static function path($req = null, $echo = true) {
		...
		if ($req === 'favicon' || $req === 'ARIO') {
			...
			$path = self::$path . PS . 'sliq';
		} elseif ($req === 'SLIQ_CSS') {
			$path = self::$path . PS . 'sliq' . PS . 'css';
		} elseif ($req === 'SLIQ_JS') {
			$path = self::$path . PS . 'sliq' . PS . 'js';
		} elseif ($req === 'SLIQ_IMAGE') {
			$path = self::$path . PS . 'sliq' . PS . 'image';
		} elseif ($req === 'AO') {
			$path = self::$path . PS . 'ao';
		} elseif ($req === 'CSS') {
