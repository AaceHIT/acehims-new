...
class ColumnQ {
	...
	public static function auth($column) {
		if ($column === 'AUTH') {
			$column = ['puid', 'tuid', 'suid', 'phone', 'username', 'email', 'password', 'verified', 'type', 'status', 'flag', 'kind', 'role', 'lastseen', 'saas', 'otp'];
		} elseif ($column === 'RECORD') {
			$column = ['puid', 'phone', 'username', 'email', 'type', 'status', 'flag', 'kind', 'role'];
		}
		return $column;
	}
