...
class DataQ {
	...
	public static function isRowColumns($data, $fields, $req = 'DATA') {
		foreach ($fields as $field) {
			if (!self::isRowColumn($data, $field, $req)) {
				return false;
			}
		}
		return true;
	...
	public static function isRowAnyColumn($data, $fields, $req = 'DATA') {
		foreach ($fields as $field) {
			if ($field === 'id' && isset($data['puid'])) {
				return true;
			} elseif ($field === 'rowid' && isset($data['suid'])) {
				return true;
			} elseif ($field === 'bind' && isset($data['tuid'])) {
				return true;
			} elseif (self::isRowColumn($data, $field, $req)) {
				return true;
			}
		}
		return false;
	...
	// ◇ ==== isNoResult →
	public static function isNoResult($record) {
		if ($record === 'NO_RESULT') {
			return true;
		}
		return false;
