	...
	var A = x.fn.datepicker.dates = {
		...
		N = {
			...
			parseDate: function(t, e, i, s) {
				function a() {
					var t = this.slice(0, n[h].length),
						e = n[h].slice(0, t.length);
				...
				if (t in c && (t = c[t]), /^[\-+]\d+[dmwy]([\s,]+[\-+]\d+[dmwy])*$/i.test(t)) {
					for (n = t.match(/([\-+]\d+)([dmwy])/gi), t = new Date, h = 0; h < n.length; h++) o = n[h].match(/([\-+]\d+)([dmwy])/i), r = Number(o[1]), l = d[o[2].toLowerCase()], t = k.prototype[l](t, r);
				...
				D.yy = D.yyyy, D.M = D.MM = D.mm = D.m, D.dd = D.d, t = F();
				var m = e.parts.slice();
				if (n.length !== m.length && (m = x(m).filter(function(t, e) {
						return -1 !== x.inArray(e, g)
					}).toArray()), n.length === m.length) {
					var y, v, w;
					for (h = 0, y = m.length; h < y; h++) {
						if (u = parseInt(n[h], 10), o = m[h], isNaN(u)) switch (o) {
							case "MM":
								p = x(A[i].months).filter(a), u = x.inArray(p[0], A[i].months) + 1;
								break;
							case "M":
								p = x(A[i].monthsShort).filter(a), u = x.inArray(p[0], A[i].monthsShort) + 1
						}
						f[o] = u
					}
					for (h = 0; h < g.length; h++)(w = g[h]) in f && !isNaN(f[w]) && (v = new Date(t), D[w](v, f[w]), isNaN(v) || (t = v))
				}
				return t
			},
